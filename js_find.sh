...
scan_js() {
    ...
    echo "${domain}" | katana | grep "\.js$" | httpx -mc 200 | tee "${base_dir}/js_1.txt"
    echo "${domain}" | gau | grep "\.js$" | httpx -mc 200 | tee "${base_dir}/js_1.txt"
    ...
    grep --color=always -i -E — "admin|auth|api|jenkins|corp|dev|stag|stg|prod|sandbox|swagger|aws|azure|uat|test|vpn|cms" "${base_dir}/js_1.txt" >> important_http_urls.txt
    grep --color=always -i -E — "aws|s3" "${base_dir}/js_1.txt" >> aws_s3_files.txt
